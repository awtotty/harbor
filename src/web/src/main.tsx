...
import type { CapabilityBundle, EnvEntry, HarborEventLog, HarborSession, ModelOption, PiPackage, Provider, SelectedModel, SystemStatus, Tab, TelegramConfig, TerminalInfo, Theme, UpdateStatus } from './types';
...
function App() {
  ...
  const { sessions, setSessions, activeSessionUpdatedAt, loadSessions } = useSessions(authed, activeSessionId, setActiveSessionId);
  useEffect(() => {
    localStorage.removeItem('harborToken');
    void fetch('/api/session').then((res) => setAuthed(res.ok)).finally(() => setCheckingAuth(false));
  }, []);
  useEffect(() => { if (authed) { void loadTerminals(); } }, [authed]);
  ...
  async function closeActiveTerminal() {
    ...
    await fetch(`/api/terminals/${activeTerminalId}`, { method: 'DELETE' });
  ...
  if (checkingAuth) return <main className={`login theme-${theme}`}><div className="brand"><span>H</span><h1>Harbor</h1></div><p>Checking session…</p></main>;
  if (!authed) return <main className={`login theme-${theme}`}><div className="brand"><span>H</span><h1>Harbor</h1></div><form onSubmit={login}><input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} /><button>Enter</button></form><p>Default password: harbor</p></main>;

  return <main className={`shell theme-${theme}`}><aside><div className="brand"><span>H</span><div><h1>Harbor</h1></div></div><div className="sessionsNav"><div className="sessionsTitle"><strong>Sessions</strong><button title="New session" onClick={newSession}>+</button></div>{sessions.map((session) => <button key={session.id} className={activeSessionId === session.id && tab === 'chat' ? 'active sessionButton' : 'sessionButton'} onClick={() => { setActiveSessionId(session.id); setTab('chat'); }}><span>{session.name}</span><small><span className="sessionTime">{formatRelativeTime(session.updatedAt)}</span><SessionTags session={session} /></small></button>)}<div className="sessionsTitle terminalsTitle"><strong>Terminals</strong><button title="New terminal" onClick={newTerminal}>+</button></div>{terminals.map((terminal) => <button key={terminal.id} className={activeTerminalId === terminal.id && tab === 'terminal' ? 'active sessionButton terminalButton' : 'sessionButton terminalButton'} onClick={() => { setActiveTerminalId(terminal.id); setTab('terminal'); }}><span>{terminal.name}</span><small>{terminal.alive ? 'open' : 'closed'}</small></button>)}</div><nav><ThemeSelector theme={theme} setTheme={setTheme} />{(['config', 'system'] as Tab[]).map((name) => <button className={tab === name ? 'active' : ''} onClick={() => setTab(name)} key={name}>{name}</button>)}<button onClick={logout}>logout</button></nav></aside><div className="content">{tab === 'chat' && (activeSessionId ? <Chat sessionId={activeSessionId} activeSessionUpdatedAt={activeSessionUpdatedAt} sessions={sessions} onSessionActivity={loadSessions} onSwitchSession={(nextSessionId) => { setActiveSessionId(nextSessionId); setTab('chat'); }} onArchiveSession={archiveActiveSession} canArchive={sessions.length > 0} /> : <NoSessions onNewSession={newSession} />)}{tab === 'terminal' && <Suspense fallback={<section className="terminalScreen noSessionsScreen"><div className="empty"><h3>Loading terminal…</h3></div></section>}><Terminal terminalId={activeTerminalId} onClose={closeActiveTerminal} onNewTerminal={newTerminal} /></Suspense>}{tab === 'config' && <Config />}{tab === 'system' && <System />}</div></main>;
}
...
function Config() { return <section className="settingsScreen"><div className="screenHeader"><h2>Config</h2><p>Manage model providers, selected model, channels, Pi packages, and environment.</p></div><Providers /><Models /><TelegramSettings /><Packages /><Env /></section>; }
...
function Providers() {
  ...
  const load = () => fetch('/api/providers').then((r) => r.ok ? r.json() : { providers: [] }).then((d) => setProviders(Array.isArray(d.providers) ? d.providers : []));
  useEffect(() => { void load(); }, []);

  async function startLogin(id: string) {
    setLog('');
    const res = await fetch(`/api/providers/${id}/login`, { method: 'POST' });
  ...
  async function logout(id: string) { await fetch(`/api/providers/${id}/logout`, { method: 'POST' }); await load(); }
  async function submitManual() { if (!manualLoginId) return; await fetch(`/api/login-input/${manualLoginId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ value: manualValue }) }); setManualLoginId(undefined); setManualValue(''); }
...
function Models() {
  ...
  const load = () => fetch('/api/models').then((r) => r.ok ? r.json() : { models: [] }).then((d) => { setModels(Array.isArray(d.models) ? d.models : []); setSelected(d.selectedModel); });
  useEffect(() => { void load(); }, []);
  async function save(value: string) { const index = value.indexOf('/'); if (index === -1) return; const provider = value.slice(0, index); const id = value.slice(index + 1); await fetch('/api/models/select', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ provider, id }) }); await load(); }
...
function TelegramSettings() {
  ...
  const load = () => fetch('/api/telegram').then((r) => r.ok ? r.json() : {}).then((d: TelegramConfig) => { setTelegram(d); setAllowedUsers((d.allowedUsers ?? []).join('\n')); });
  useEffect(() => { void load(); }, []);
  async function save() {
    setError('');
    const res = await fetch('/api/telegram', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ enabled: telegram.enabled, botToken, allowedUsers }) });
    const data = await res.json();
    setTelegram(data);
    setBotToken('');
  ...
  async function testToken() {
    setError('');
    const res = await fetch('/api/telegram/test', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ botToken }) });
    const data = await res.json();
  ...
  async function useSender(senderId: string) {
    setError('');
    const res = await fetch('/api/telegram/allow-user', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: senderId }) });
    const data = await res.json();
...
function Packages() {
  ...
  const load = () => Promise.all([
    ...
    fetch('/api/bundles').then((r) => r.ok ? r.json() : { bundles: [] }),
...
function Env() {
  ...
  useEffect(() => { void fetch('/api/env').then((r) => r.ok ? r.json() : { entries: [] }).then((d) => setEntries(Array.isArray(d.entries) ? d.entries : [])); }, []);
  async function save() { await fetch('/api/env', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ entries }) }); alert('Saved'); }
...
function System() {
  ...
  async function load() {
    const [statusRes, systemsRes, eventsRes, updatesRes] = await Promise.all([
      ...
      fetch('/api/observability/events?limit=80'),
